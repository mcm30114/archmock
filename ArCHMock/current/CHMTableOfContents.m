...
- (id)initWithContainer:(CHMContainer *)container {
    if (self = [super init]) {
        NSString *tableOfContentsPath = [container findMetadataStringInSystemObjectWithOffset:0
                                                                  orInStringsObjectWithOffset:0x60];
...
//        NSLog(@"DEBUG: Populating table of contents");
        ...
        NSArray *rootListItems = [doc nodesForXPath:@"html/body/ul/li"
                                              error:&error];
        ...
        while ([listItemsQueue count] > 0) {
            ...
            if ([listItemsStack count] > 0 
                && listItemElement == [listItemsStack objectAtIndex:0]) {
                ...
                NSXMLElement *sectionElement = [[listItemElement nodesForXPath:@"object[@type='text/sitemap']"
                                                                       error:&error] lastObject];
                //                NSLog(@"DEBUG: section element: %@", sectionElement);
                NSXMLNode *labelAttribute = [[sectionElement nodesForXPath:@"param[@name='Name' or @name='name']/@value"
                                                                  error:&error] lastObject];
                NSXMLNode *pathAttribute = [[sectionElement nodesForXPath:@"param[@name='Local' or @name='local']/@value"
                                                                  error:&error] lastObject];
                CHMSection *section = [CHMSection sectionWithLabel:[labelAttribute stringValue] 
                                                              path:[pathAttribute stringValue]
                                                            parent:currentParentSection];
                ...
                NSArray *childrenListItems = [listItemElement nodesForXPath:@"ul/li"
                                                                      error:&error];
                if ([childrenListItems count] > 0) {
                    if (1 == [childrenListItems count]) {
                        [listItemsQueue insertObject:[childrenListItems objectAtIndex:0] 
                                             atIndex:0];
                    }
                    else {
                        [listItemsQueue insertObjects:childrenListItems
                                            atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, [childrenListItems count])]];
                    }
                }
            }
        }
